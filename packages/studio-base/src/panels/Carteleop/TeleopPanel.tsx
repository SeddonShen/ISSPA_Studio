...
function TeleopPanel(props: TeleopPanelProps): JSX.Element {
  ...
  const [config, setConfig] = useState<Config>(() => {
    ...
    const {
      topic,
      publishRate = 10,
      upButton: { field: upField = "linear-x", value: upValue = 0.2 } = {},
      downButton: { field: downField = "linear-x", value: downValue = -0 } = {},
      leftButton: { field: leftField = "angular-z", value: leftValue = 0.3 } = {},
      rightButton: { field: rightField = "angular-z", value: rightValue = -0.3 } = {},
    } = partialConfig;
  ...
  useLayoutEffect(() => {
    ...
    switch (currentAction) {
      ...
      case DirectionalPadAction.STOP:
        setFieldValue(config.upButton.field, 0);
        setFieldValue(config.leftButton.field, 0);
        setFieldValue(config.rightButton.field, 0);
        setFieldValue(config.downButton.field, 0);
        break;
      case DirectionalPadAction.REVERSELEFT:
        setFieldValue(config.downButton.field, config.downButton.value);
        setFieldValue(config.leftButton.field, config.leftButton.value);
        break;
      case DirectionalPadAction.REVERSERIGHT:
        setFieldValue(config.downButton.field, config.downButton.value);
        setFieldValue(config.rightButton.field, config.rightButton.value);
        break;
      default:
    ...
    console.log("publishing", message, "to", currentTopic, "at", intervalMs, "ms");
